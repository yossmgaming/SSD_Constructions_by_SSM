...
export default function Reports() {
    ...
    async function loadData() {
        ...
        try {
            ...
            const [projs, wrks, mats, sups, pays, atts, advs] = await Promise.all([
                ...
                queryAdvanced(KEYS.attendances, { filters: payFilter, select: 'id, workerId, projectId, isPresent, hoursWorked, isHalfDay, hours, status, date' }),
                queryAdvanced(KEYS.advances, { filters: advFilter, select: 'id, amount, projectId, workerId, date' })
            ]);
            setProjects(projs);
            setWorkers(wrks);
            setMaterials(mats);
    ...
    // ─── Worker report data ───────────────────────────────────
    const { data: workerReport, isComputing: isComputingWorkerReport } = useAsyncData(async () => {
        // Pre-index data for O(1) lookups inside the loop
        const paymentsMap = new Map();
        payments.forEach(p => {
            if (!paymentsMap.has(p.workerId)) paymentsMap.set(p.workerId, []);
            paymentsMap.get(p.workerId).push(p);
        });

        const advancesMap = new Map();
        advances.forEach(a => {
            if (!advancesMap.has(a.workerId)) advancesMap.set(a.workerId, []);
            advancesMap.get(a.workerId).push(a);
        });

        const attendanceMap = new Map();
        allAttendance.forEach(a => {
            if (!attendanceMap.has(a.workerId)) attendanceMap.set(a.workerId, []);
            attendanceMap.get(a.workerId).push(a);
        });

        return workers.map((w) => {
            const workerPay = (paymentsMap.get(w.id) || []).filter((p) => p.category === 'Worker Pay');
            const totalPaid = workerPay.reduce((s, p) => s + (p.amount || 0), 0);
            const workerAdv = advancesMap.get(w.id) || [];
            ...
            attendance.forEach((a) => {
                ...
                if (a.isPresent !== undefined) {
                    ...
                    else if (a.status === 'Absent') return;
                }
    ...
    // ─── Performance metrics ───
    const { data: perfData, isComputing: isComputingPerfData } = useAsyncData(async () => {
        ...
        return workers.filter(w => w.status === 'Active').map(w => {
            ...
            attendance.forEach(a => {
                ...
                if (a.isPresent !== undefined) {
                    ...
                    if (a.hours) hours = parseFloat(a.hours);
                    else if (a.status === 'Present') hours = 8;
                    else if (a.status === 'Half') hours = 4;
                    else if (a.status === 'Absent') return;
                }
            ...
            const workerPay = (paymentsMap.get(w.id) || []).filter(p => p.direction === 'Out');
            const paid = workerPay.filter(p => p.category === 'Worker Pay').reduce((s, p) => s + (p.amount || 0), 0);
            const adv = (advancesMap.get(w.id) || []).reduce((s, a) => s + (a.amount || 0), 0);
    ...
    const { data: projectReport, isComputing: isComputingProjectReport } = useAsyncData(async () => {
        // Pre-indexing
        const paymentsMap = new Map();
        payments.forEach(p => {
            if (!paymentsMap.has(p.projectId)) paymentsMap.set(p.projectId, []);
            paymentsMap.get(p.projectId).push(p);
        });
        const advancesMap = new Map();
        advances.forEach(a => {
            if (!advancesMap.has(a.projectId)) advancesMap.set(a.projectId, []);
            advancesMap.get(a.projectId).push(a);
        });

        return projects.map(p => {
            const projPay = paymentsMap.get(p.id) || [];
            const projAdv = advancesMap.get(p.id) || [];
    ...
    const isGlobalLoading = isLoading || isComputingWorkerReport || isComputingPerfData || isComputingProjectReport;

    return (
        <GlobalLoadingOverlay loading={isGlobalLoading} message="Synthesizing Construction Intelligence...">
