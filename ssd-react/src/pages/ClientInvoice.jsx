...
export default function ClientInvoice() {
    ...
    const [form, setForm] = useState({ clientName: '', projectId: '', totalAmount: '', dueDate: '', status: 'Draft', notes: '' });
    ...
    async function handleSave() {
        const data = {
            type: 'ClientInvoice', direction: 'Receivable',
            entityType: 'Client', clientName: form.clientName,
            projectId: form.projectId ? parseInt(form.projectId) : null,
            totalAmountSnapshot: parseFloat(form.totalAmount) || 0,
            dueDate: form.dueDate, status: form.status, notes: form.notes,
        };
        setIsLoading(true);
        try {
    ...
    function handleClear() {
        setForm({ clientName: '', projectId: '', totalAmount: '', dueDate: '', status: 'Draft', notes: '' });
        setSelectedId(null);
        setSettlements([]);
    }
    ...
    const columns = [
        ...
        { key: 'dueDate', label: 'Due', render: (v) => v ? new Date(v).toLocaleDateString() : '-' },
        {
            key: 'status', label: 'Status', render: (v) => (
                <span className={`badge ${v === 'Settled' ? 'badge-success' : v === 'Partial' ? 'badge-warning' : v === 'Overdue' ? 'badge-danger' : 'badge-default'}`}>{v}</span>
            )
        },
    ];
