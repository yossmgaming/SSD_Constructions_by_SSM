    ...
    async function loadOverview() {
        ...
        try {
            ...
            allAttendance.forEach((a) => {
                ...
                if (!workerAttendance[name]) workerAttendance[name] = { days: 0, rate: w?.dailyRate || 0 };

                // 1. Check for new UI fields
                if (a.isPresent !== undefined) {
                    if (!a.isPresent) return;
                    if (a.hoursWorked) workerAttendance[name].days += parseFloat(a.hoursWorked) / 8;
                    else if (a.isHalfDay) workerAttendance[name].days += 0.5;
                    else workerAttendance[name].days += 1;
                }
                // 2. Fallback to legacy
                else {
                    if (a.hours) workerAttendance[name].days += parseFloat(a.hours) / 8;
                    else if (a.status === 'Present' || a.status === 'Full') workerAttendance[name].days += 1;
                    else if (a.status === 'Half' || a.status === 'Half Day') workerAttendance[name].days += 0.5;
                }
            });
                                ...
                                {Object.entries(overview.workerAttendance).length > 0 ? (
                                    Object.entries(overview.workerAttendance).map(([name, data]) => (
                                        <div key={name} className="summary-bar">
                                            ...
                                            <span>{Math.round(data.days * 10) / 10} days × {fmt(data.rate)} = <strong>{fmt(data.days * data.rate)}</strong></span>
                                        </div>
                                    ))
